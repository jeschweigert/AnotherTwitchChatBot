...
namespace ATCB.Library.Models.Commands.Discord
{
    ...
    {
        private string discordOAuth = "https://discordapp.com/api/oauth2/authorize?client_id=426112428778979359&permissions=70765648&redirect_uri=http%3A%2F%2Fsandhead.stream%2Fbot%2Fapi%2Fdiscord_auth.php&response_type=code&scope=bot%20identify%20messages.read%20connections";
                ...
                else if (context.ArgumentsAsList[0] == "friendalerts" && context.ChatMessage.IsChatBot)
                {
                    if (context.ArgumentsAsList[1] == "set")
                    {
                        ...
                        if (channel != null)
                        {
                            ...
                            context.Settings.Discord.TwitchFriendLiveAnnounceTextChannel = channel.Id;
                            context.Settings.Save();
                        }
                        else
                        {
                    ...
                    else if (context.ArgumentsAsList[1] == "get")
                    {
                        var channel = context.DiscordClient.GetChannels().Where(x => x.Id == context.Settings.Discord.TwitchFriendLiveAnnounceTextChannel).FirstOrDefault();
                        context.SendMessage($"The currently set channel for friend alerts is #{channel.Name}.");
                    }
                }
            }
